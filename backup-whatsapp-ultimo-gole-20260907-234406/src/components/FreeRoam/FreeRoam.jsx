          ...
          <section className="free-roam-actions">
            {shelteredDuringDay ? (
              ...
              <>
                ...
                {getDistrictPlaces(game, locationId).map(place => <button type="button" key={place.id} disabled={blocked || Boolean(prey || pendingHumanity || stoppingFeeding || frenzyActive || masqueradeEvent)} onClick={() => {
                  const travel = calculateTravel(game, place.id, 'walking')
                  if (travel.allowed) onTravel(travel)
                  else setMessage(travel.reason)
                }}><strong>{place.name}</strong><span>Ir até o local · {calculateTravel(game, place.id, 'walking').minutes} min a pé</span></button>)}
