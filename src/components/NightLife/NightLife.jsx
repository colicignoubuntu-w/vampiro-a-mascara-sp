...
export default function NightLife({ game, onChange, blocked, compact = false }) {
  ...
  const selected = tab || (compact ? 'jobs' : 'agenda')
  ...
  return <>
    ...
    {open && createPortal(<div className="night-backdrop" onClick={close}>
      <section className="night-dialog" role="dialog" aria-modal="true" aria-labelledby="night-title" tabIndex={-1} ref={dialog} onKeyDown={trap} onClick={e => e.stopPropagation()}>
        ...
        <nav aria-label="Seções da agenda">{(compact ? [['jobs', 'Trabalhos'], ['crime', 'Atividades de risco']] : [['agenda', 'Agenda'], ['jobs', 'Trabalhos'], ['crime', 'Atividades de risco'], ['phone', `Celular (${unread})`]]).map(([id, label]) => <button type="button" key={id} aria-pressed={selected === id} onClick={() => setTab(id)}>{label}</button>)}</nav>
