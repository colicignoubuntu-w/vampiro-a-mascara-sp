...
export default function Game({
  ...
  */
  ...
  function handleCombatDiscipline(actionId) {
    if (!combat || combat.status !== 'active') return
    const result = performCombatDisciplineAction({ game, combat, actionId })
    if (!result.success) {
      if (result.log?.[0]?.text) window.alert(result.log[0].text)
      return
    }
    const nextCombat = {
      ...result.combat,
      log: [ ...(result.combat?.log ?? combat.log ?? []), ...(result.log ?? []) ],
    }
    const updatedGame = {
      ...result.game,
      combat: nextCombat,
      history: [
        ...(result.game.history ?? []),
        { type: 'combat-discipline-action', scene: scene.id, encounter: combat.encounterId, round: combat.round, action: actionId, timestamp: new Date().toISOString() },
      ],
    }
    persist(updatedGame)
  }
